...
A comprehensive, production-ready Municipal Complaint Management System built for Government of India e-Governance compliance, following NIC/MeitY architectural standards.

**Status:** ✅ All Core Features Working | 19/19 Tests Passing | Django 6.0.2

---

## 📋 Quick Navigation

- **Setup:** See [Installation & Quick Start](#installation--quick-start) below
- **Testing:** Run `python manage.py test` (19 automated tests)
- **Development:** `python manage.py runserver` at http://127.0.0.1:8000
- **Issues Fixed:** See [Recent Fixes](#recent-fixes) section
...
### Prerequisites
- Python 3.8+ (Anaconda/standard Python)
...
for code, name in [('WATER_SUPPLY', 'Water Supply'), ('ROADS_TRANSPORT', 'Roads & Transport')]:
...
### Run All Tests (19 automated tests)
```bash
python manage.py test
# Expected: Ran 19 tests in ~12s ... OK
```
...
### Issue 1: Missing Department Detail Template ✅ FIXED
**Problem:** `/departments/PUBLIC_HEALTH/` threw `TemplateDoesNotExist: departments/detail.html`

**Solution:** Created `templates/departments/detail.html` to render department info, categories, and "File a Complaint" button.

### Issue 2: Dynamic Category Loading ✅ FIXED
**Problem:** Complaint form didn't update categories when department was selected.

**Solution:** Added JavaScript in `complaints/submit.html` to fetch categories dynamically via AJAX `/complaints/ajax/load-categories/?department_id=<code>`.

### Issue 3: Department Pre-selection ✅ FIXED
**Problem:** No way to pre-fill department when filing complaints from department pages.

**Solution:** Added URL parameter `?department=<code>` support; `submit.html` now auto-selects department from query string.

### Issue 4: File Upload Feedback ✅ FIXED
**Problem:** Users didn't see feedback after selecting a proof file.

**Solution:** Added JavaScript to display selected file name and size in real-time.

### Issue 5: CAPTCHA Refresh ✅ VERIFIED
**Status:** `/accounts/refresh-captcha/` and CAPTCHA generation working correctly.
